<script>
  ...
  // Datos que vienen desde TimeLine
  export let username;
  export let location;
  export let photo;
  ...
  export let comments;
  export let avatar;
  // Función para abrir o cerrar modal
  let isModal = false;
  const handleClick = () => {
    isModal = !isModal;
  };
  ...
  const handleLike = () => {
    like = !like;
    // Agregar o restar el número de likes en el store
    if (like) {
